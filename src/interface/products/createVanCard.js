...
export function createVancard(cardInfo) {
  ...
  textModal.innerHTML=`<div class="nameModal"><h2>${cardInfo.name}</h2></div><div class="priceModal"><h2>Precio</h2><p>${cardInfo.price}</p> </div><div class="descripcionModal"><h2>Descripcion</h2><p>${cardInfo.descripcion}</p> </div><div class="buttonsModal"><a  href="thankYou.html"><button style="cursor:pointer" id="buyModalButton" type="button">Comprar</button></a><button style="cursor:pointer"class="cartModalButton${cardInfo.codigo}">Carrito</button></div>`;
