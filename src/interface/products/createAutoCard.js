import { autos_Section } from "../../utils/nodes.js";

export function createAutoCard(cardInfo) {
  ...
  imgCard.setAttribute('src',`public/imgProducts/autos/${cardInfo.url}`);
 ...
 //Creaing a modal for each product
  ...
  figureModal.innerHTML=`<img class="imgModal" src='public/imgProducts/autos/${cardInfo.url}'/ >`;
  modalContent.appendChild(figureModal);

  const textModal=document.createElement('div');
  textModal.setAttribute('class','textModal');
  textModal.innerHTML=`<div class="nameModal"><h2>${cardInfo.name}</h2></div><div class="priceModal"><h2>Precio</h2><p>S/.${cardInfo.price}</p> </div><div class="descripcionModal"><h2>Descripcion</h2><p>${cardInfo.descripcion}</p> </div><div class="buttonsModal"><a href="thankYou.html"><button id="buyModalButton"  style="cursor:pointer" type="button">Comprar</button></a><button style="cursor:pointer"class="cartModalButton${cardInfo.codigo}">Carrito</button></div>`;
  modalContent.appendChild(textModal); 
 //Creating a modal finishes
 
  card.appendChild(containerCardDetails);
  autos_Section.appendChild(card);
